...
class Index extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      cats: [{ _id: "All Posts", name: "All Posts" }],
      posts: [],
      postsToShow: [],
      loading: "Loading",
    };
  ...
  filterPosts(e) {
    ...
    const postsToShow = posts.filter((post) => {
      ...
      return e.target.name == "All Posts"
        ? true
        : post.possibleBadges.some((badge) => badge == e.target.name);
    });
    ...
    await axios
      .get(
        "https://localhost:8080/" +
          (this.state.next
            ? "?next=" + encodeURIComponent(this.state.next)
            : "")
      )
      .then((res) => {
        ...
        posts = posts.map((post) => {
          ...
          if (
            post.scores.length &&
            Object.keys(post.scores[0]).length &&
            post.scores[0][Object.keys(post.scores[0])[0]]
          ) {
              ...
              {(this.state.cats.length &&
                this.state.cats.map((cat, i) => (
                  <a
                    ...
                    name={cat._id}
                    className={`badge badge-${cat.color}`}
                  >
                    {cat.name}
                  </a>
                ))) || <div className="text-center">{this.state.loading}</div>}
            </div>
          </div>
          <div className="container">
            <div className="row margin-top">
              {(this.state.postsToShow.length &&
                this.state.postsToShow.map(
                  (post, i) =>
                    post.message && (
                      <Post
                        key={i}
            ...
            <div className="text-center">
              {this.state.next && this.state.loading != "Loading" && (
                <a
