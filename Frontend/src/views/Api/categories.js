    ...
    if (
      window.confirm(
        "Are you sure you want to delete " +
          e.target.name +
          " category and all it's word lists?"
      )
    ) {
      const axios = require("../../utils/axios");
      axios
        .post("https://localhost:8080/cats/delete", {
          id: e.target.getAttribute("cat"),
        })
        .then((e) => {
          if (e.data.success) {
            ...
            <tbody>
              {this.state.cats.map((cat, i) => (
                <tr key={i}>
                  <td>{cat.name}</td>
                  <td>
                    <a href="#" className={`badge badge-${cat.color}`}>
                      {colorMap[cat.color] || "White"}
                    </a>
                  </td>
                  <td>
                    <Link to={"/admin/categories/cat?id=" + cat._id} cat={cat}>
