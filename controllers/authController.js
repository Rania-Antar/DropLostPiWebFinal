...
const forgotPassword = async (req, res) => {
  ...
  res.send({
    success: {
      global: `a reset password e-mail was sent to ${user.email} please check it out`,
    },
  });
};
...
const signInController = async (req, res) => {
  ...
  if (!user.isActive) {
    return res.status(400).send({
      errors: {
        global:
          "your account is inactive, please check your e-mail to actviate it",
        userId: user._id,
      },
    });
...
const signUpController = async (req, res) => {
  ...
  res.send({
    success: {
      global: `a verification e-mail was sent to ${user.email} please check it out`,
    },
  });
};
...
const resendAccountActivationEmailController = async (req, res) => {
  ...
  if (!token) {
    return res.status(400).send({
      errors: {
        global:
          "you have no account activation token, please contact our administration",
      },
    });
  }
